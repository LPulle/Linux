...
# install using a for loop so will keep going even with an error
for i in $(cat ~/pipfreeze.txt); do sudo /usr/local/bin/pip3.7 install $i; done
for i in $(cat ~/pipfreeze.txt); do sudo pip3 install $i; done
for i in $(cat ~/pipfreeze.txt); do sudo pip2 install $i; done
